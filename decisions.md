...
**Sessions render as condensed, tap-to-expand rows with R/V letter chips, not full cards.**
The browse list (both day tabs and all-days) now shows each session as a row that leads with the title (so all titles align flush left), a muted second line with presenter, room, and (on all-days) start time, and the chips moved to the right: a gold "R" when required for the viewer and a teal "V" when virtual. A "+" adds it; tapping the row expands it in place to show time, location/join link, presenter, audience, requirement detail, repeat note, and description. Dividers were strengthened so rows do not blur together. A legend row under the day selector explains the chips (R first, then V), and in the expanded detail the "Where" line is teal and the "Requirement" line is amber, each colored only when the matching chip applies, so the link between a marker and its detail is explicit. The Join link continues to come from the Sheet's "Virtual Link" column, the structured home for it. To avoid duplication, the condensed second line (presenter, room) is hidden once a row is expanded, since the open detail already covers it; and the "Also offered" line is a pale navy pill (FCPS navy accent on a light tint) rather than teal, so it stands out as its own element while teal stays exclusively the virtual marker and amber the required marker. (Plain navy text alone blended into the dark labels; the pill background is what makes it pop.) Reason: even after the role filter, the core breakout blocks hold 16 to 21 relevant sessions for one person (and there are about seven such blocks across the week), and there is no Theme/Track data coming to subdivide them. Full cards made those blocks an unscrollable wall; condensed rows fit a whole block on one screen. Letter chips were chosen over a star/dot because a colored "R"/"V" with a slight shadow reads clearly as a label. The old `cardFor` renderer is retained but unused, so reverting or switching to condense-only-large-blocks is a one-line change.

**Day tabs are grouped into collapsible time blocks; the all-days view stays grouped by day.**
On a single-day tab, sessions are now bucketed by their normalized start time, each block a collapsible section (open by default) styled like the day headers, ordered chronologically, with a "Time to be announced" block at the bottom for sessions that have no usable time yet. A session appears once, under its start time. A session that runs long and overlaps a later block is not duplicated into that block: its end time is on the card, and real clashes are still caught by the personal-schedule conflict warning. Reason: each day is really a few large concurrent breakout blocks, and most afternoon breakouts run twice via "Also Offered At" (Tue 1:00 PM and 2:15 PM, about 23 each; Wed 10:00 and 11:15; Thu 12:30 and 1:45), so a flat per-day list of well over 40 occurrences was unreadable. The grouping buckets the already-expanded occurrence list, so each repeat lands in its own block automatically. Combined with the role auto-filter, a 22-session block collapses to just the handful relevant to the viewer. The collapsible-section caret was also made more prominent (larger teal chevron in a soft rounded background that darkens on hover).

**Time parsing tolerates seconds and infers PM for bare afternoon hours.**
`timeToMin` now accepts an optional seconds part ("1:00:00 PM") and, when a time carries no am/pm, treats bare hours 1 through 6 as afternoon (PM). Reason: the live Sheet had about 37 sessions whose start cells were formatted with seconds, which the old parser could not read, so they showed no time and sank to the bottom; and several afternoon times were entered without am/pm ("1:45", "2:15"), which the old parser read as early morning (the "2:15 AM" seen on a card). The conference is entirely daytime, so 1 through 6 without a suffix can only mean PM. Format variants now normalize to the same minute, so "1:00 PM", "1:00:00 PM", and "1:00" all merge into one block. Entering explicit am/pm in the Sheet is still welcome but no longer required.

**Keep the cache warm with a 5-minute trigger, and guard rebuilds with a lock.**
Measured live `doGet` durations: about 0.8 seconds on a cache hit, about 3.4 seconds on a cold rebuild that reads the Sheet. `getData` now rebuilds under a `LockService` script lock, so a cold cache cannot cause many simultaneous Sheet reads: one execution rebuilds and caches while others wait up to 5 seconds and then read the now-warm cache (if the lock is not obtained, it serves a fresh uncached build rather than blocking). A time-based trigger, `warmCache`, installed once via `installCacheWarmer()`, refreshes the cache every 5 minutes so it never expires during active hours and every visitor stays on the fast 0.8s path. Reason: IT (Noah Hwang) asked about the 30-simultaneous-executions-per-user limit (the app runs as the owner account, so all visitors share one account's quota). With the warm path, reaching 30 would require roughly 38 requests per second, meaning all 1,500 attendees loading within about 40 seconds; the lock plus keepalive remove the cold-rebuild stampede that was the only realistic way to approach it. Apps Script does not queue requests past the limit, so this prevention-by-staying-warm is the in-architecture answer rather than queueing. Verified with a mock-LockService test.

**Cache each day under its own key, not the whole payload under one key.**
`getData()` now stores each day tab (plus help and the audience map) under a separate CacheService key and reads them back in one `getAll`, writes in one `putAll`, and clears them in one `removeAll`. Because the 100KB limit is per key, this gives each day its own budget instead of one shared 100KB ceiling. The measured live payload today is about 14KB of schedule (the largest single day is roughly 4.4KB), so this leaves effectively unlimited headroom as detailed descriptions are added. All keys are written together with the same 10-minute TTL, and if any key is missing (expired, or a piece ever exceeds the per-key cap) the whole payload is rebuilt once and re-cached, so a load never mixes data of different ages. `CACHE_VERSION` ('v3') lets us invalidate every key at once. Verified with a mock-CacheService test (cold build, warm hit, `?fresh=1` bypass, flush, and oversized-day fallback). Reason: IT (Noah Hwang) flagged that a single cached value goes invalid past 100KB; per-key caching removes that ceiling with no change to what the app receives.
